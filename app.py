...
def clean_pdf_text(text):
    """Supprime les caractères spéciaux non supportés par FPDF latin-1"""
...
with tabs[1]:
    if not st.session_state.planning.empty:
        ...
        if sel_a:
            # --- ZONE INFOS TECHNIQUES (CHAMPS TEXTE) ---
            ...
            with it1: st.session_state.infos_techniques[sel_a]["entrees"] = st.text_input("Circuits d'entrées", st.session_state.infos_techniques[sel_a]["entrees"], key=f"it_ent_{sel_a}")
            with it2: st.session_state.infos_techniques[sel_a]["ear"] = st.text_input("EAR MONITOR", st.session_state.infos_techniques[sel_a]["ear"], key=f"it_ear_{sel_a}")
            with it3: st.session_state.infos_techniques[sel_a]["stereo"] = st.text_input("MONITOR // circuits stéréo", st.session_state.infos_techniques[sel_a]["stereo"], key=f"it_st_{sel_a}")
            with it4: st.session_state.infos_techniques[sel_a]["mono"] = st.text_input("MONITOR // circuits mono", st.session_state.infos_techniques[sel_a]["mono"], key=f"it_mo_{sel_a}")
            st.write("---")
            ...
            with col_patch:
                ...
                if st.session_state[f"ed_patch_{sel_a}"]["deleted_rows"]:
                    st.session_state.fiches_tech = st.session_state.fiches_tech.drop(df_patch_art.index[st.session_state[f"ed_patch_{sel_a}"]["deleted_rows"][0]]).reset_index(drop=True)
                    st.rerun()
    ...
    with cex2:
        ...
        with st.container(border=True):
            ...
            if st.button("Générer PDF Besoins", use_container_width=True):
                ...
                # --- FIX : AJOUT INFOS TECHNIQUES DANS LE PDF ---
                if sel_grp_exp != "Tous" and sel_grp_exp in st.session_state.infos_techniques:
                    ...
                    dico_besoins["--- CONFIGURATION TECHNIQUE ---"] = pd.DataFrame([
                ...
                if m_bes == "Par Jour & Scène":
                    ...
                    if not data_pic.empty:
                        for cat in data_pic["Catégorie"].unique():
                            # FIX KeyError : Sélection sécurisée des colonnes
