...
st.set_page_config(page_title="Regie-Festival", layout="wide", initial_sidebar_state="collapsed") [cite: 1]
...
if 'planning' not in st.session_state:
    st.session_state.planning = pd.DataFrame(columns=["Scène", "Jour", "Artiste", "Balance", "Durée Balance", "Show"]) [cite: 2]
if 'fiches_tech' not in st.session_state:
    st.session_state.fiches_tech = pd.DataFrame(columns=["Scène", "Jour", "Groupe", "Catégorie", "Marque", "Modèle", "Quantité", "Artiste_Apporte"]) [cite: 2]
if 'riders_stockage' not in st.session_state:
    st.session_state.riders_stockage = {} [cite: 2]
if 'artist_circuits' not in st.session_state:
    st.session_state.artist_circuits = {} [cite: 2]
if 'patch_data' not in st.session_state:
    st.session_state.patch_data = {}
if 'uploader_key' not in st.session_state:
    st.session_state.uploader_key = 0 [cite: 2]
if 'delete_confirm_idx' not in st.session_state:
    st.session_state.delete_confirm_idx = None [cite: 2]
if 'delete_confirm_patch_idx' not in st.session_state:
    st.session_state.delete_confirm_patch_idx = None [cite: 2]
if 'festival_name' not in st.session_state:
    st.session_state.festival_name = "MON FESTIVAL" [cite: 2]
if 'festival_logo' not in st.session_state:
    st.session_state.festival_logo = None [cite: 2]
if 'custom_catalog' not in st.session_state:
    st.session_state.custom_catalog = {} [cite: 3]
...
class FestivalPDF(FPDF):
    def header(self):
        if st.session_state.festival_logo:
            try:
                ...
                with tempfile.NamedTemporaryFile(delete=False, suffix=".png") as tmp_file:
                    tmp_file.write(st.session_state.festival_logo) [cite: 4]
        ...
        offset_x = 45 if st.session_state.festival_logo else 10 [cite: 5]
    ...
    def ajouter_titre_section(self, titre):
        ...
        self.set_fill_color(240, 240, 240) [cite: 6]
    ...
    def dessiner_tableau(self, df):
        ...
        for _, row in df.iterrows():
            ...
            for item in row:
                self.cell(col_width, 6, str(item), border=1, align='C') [cite: 8]
            self.ln()
        self.ln(5)
...
def generer_pdf_complet(titre_doc, dictionnaire_dfs):
    ...
    for section, df in dictionnaire_dfs.items():
        if not df.empty:
            ...
            pdf.ajouter_titre_section(section) [cite: 9]
...
with main_tabs[0]:
    ...
    with sub_tabs_config[0]:
        ...
        with st.container(border=True): [cite: 10]
            ...
            if st.button("Valider Artiste"):
                if ar:
                    ...
                    val_du = du if opt_balance else "" [cite: 15]
                    new_row = pd.DataFrame([{
                        "Scène": sc, "Jour": str(jo), "Artiste": ar, 
                        "Balance": val_ba, "Durée Balance": val_du, "Show": sh.strftime("%H:%M") [cite: 16-17]
                    }])
                    st.session_state.planning = pd.concat([st.session_state.planning, new_row], ignore_index=True)
                    if ar not in st.session_state.riders_stockage: [cite: 18]
                        st.session_state.riders_stockage[ar] = {}
                    if pdfs:
                        for f in pdfs:
                            st.session_state.riders_stockage[ar][f.name] = f.read() [cite: 19]
                    st.session_state.uploader_key += 1
                    st.rerun()

        st.subheader("📋 Planning Global (Modifiable)")
        if st.session_state.delete_confirm_idx is not None:
            idx = st.session_state.delete_confirm_idx [cite: 20]
            with st.status("⚠️ Confirmation de suppression", expanded=True):
                ...
                if col_cfg1.button("✅ OUI, Supprimer", use_container_width=True):
                    nom_art = st.session_state.planning.iloc[idx]['Artiste'] [cite: 21]
                    ...
                    st.rerun()
                if col_cfg2.button("❌ Annuler", use_container_width=True): [cite: 22]
                    st.session_state.delete_confirm_idx = None
                    st.rerun()

        if not st.session_state.planning.empty:
            df_visu = st.session_state.planning.sort_values(by=["Jour", "Scène", "Show"]).copy() [cite: 23]
            ...
            if st.session_state.main_editor["deleted_rows"]:
                ...
                st.rerun()
            df_to_save = edited_df.drop(columns=["Rider"]) [cite: 24]
        ...
        if st.session_state.riders_stockage: [cite: 25]
            ...
            if keys_list:
                ...
                fichiers = st.session_state.riders_stockage.get(choix_art_pdf, {}) [cite: 26]
                for fname in list(fichiers.keys()):
                    cf1, cf2 = st.columns([4, 1])
                    cf1.write(f"📄 {fname}")
                    if cf2.button("🗑️", key=f"del_pdf_{fname}"): [cite: 27]
                        del st.session_state.riders_stockage[choix_art_pdf][fname]
                        st.rerun()
                with cg2:
                    nouveaux_pdf = st.file_uploader("Ajouter des fichiers", accept_multiple_files=True, key="add_pdf_extra") [cite: 28]
    ...
    with sub_tabs_config[1]:
        ...
        with col_adm1:
            ...
            with st.container(border=True):
                new_name = st.text_input("Nom du Festival", st.session_state.festival_name) [cite: 30]
            ...
            with st.container(border=True): [cite: 32]
                ...
                pickle_out = pickle.dumps(data_to_save) [cite: 34]
                ...
                if uploaded_session:
                    if st.button("Restaurer la sauvegarde"): [cite: 35]
                        data_loaded = pickle.loads(uploaded_session.read())
                        st.session_state.planning = data_loaded["planning"]
                        st.session_state.fiches_tech = data_loaded["fiches_tech"] [cite: 36]
                        ...
                        st.session_state.festival_name = data_loaded.get("festival_name", "Mon Festival") [cite: 37]
                        ...
                        st.success("Session restaurée avec succès !") [cite: 38]
                        st.rerun()
        with col_adm2: [cite: 39]
            st.subheader("📚 Catalogue Matériel (Excel)")
            code_secret = st.text_input("🔒 Code Admin", type="password")
            if code_secret == "0000":
                xls_file = st.file_uploader("Fichier Excel Items", type=['xlsx', 'xls']) [cite: 39]
                if xls_file: [cite: 40]
                    if st.button("Analyser et Charger le Catalogue"):
                        xls = pd.ExcelFile(xls_file) [cite: 41]
                        new_catalog = {}
                        for sheet in xls.sheet_names:
                            df = pd.read_excel(xls, sheet_name=sheet) [cite: 42]
                            new_catalog[sheet] = {brand: df[brand].dropna().astype(str).tolist() for brand in df.columns} [cite: 43-44]
                        st.session_state.custom_catalog = new_catalog
                        st.success("Catalogue chargé !") [cite: 45]
                if st.session_state.custom_catalog: [cite: 46]
    ...
    with sub_tabs_config[2]:
        ...
        cex1, cex2 = st.columns(2) [cite: 48]
        ...
        with cex1:
            ...
            s_s_p = st.selectbox("Scène", l_scenes) if m_plan == "Par Scène" else None [cite: 48-49]
            if st.button("Générer PDF Planning", use_container_width=True):
                dico_sections = {}
                jours_a_traiter = [s_j_p] if m_plan == "Par Jour" else l_jours
                scenes_a_traiter = [s_s_p] if m_plan == "Par Scène" else l_scenes [cite: 50]
                for j in jours_a_traiter:
                    for s in scenes_a_traiter:
                        sub_df = st.session_state.planning[(st.session_state.planning["Jour"] == j) & (st.session_state.planning["Scène"] == s)].sort_values("Show") [cite: 51]
                        if not sub_df.empty:
                            dico_sections[f"JOUR : {j} | SCENE : {s}"] = sub_df[["Artiste", "Balance", "Durée Balance", "Show"]] [cite: 52-53]
        ...
        with cex2:
            ...
            m_bes = st.radio("Type", ["Par Jour & Scène", "Total Période par Scène"], key="mb") [cite: 54]
            s_s_m = st.selectbox("Scène", l_scenes, key="ssm")
            s_j_m = st.selectbox("Jour", l_jours, key="sjm") if m_bes == "Par Jour & Scène" else None [cite: 55]
            if st.button("Générer PDF Besoins", use_container_width=True): [cite: 56]
                dico_besoins = {}
                # (Logique simplifiée pour l'export ici, à adapter selon vos besoins de calcul de pic)
                df_exp = st.session_state.fiches_tech[(st.session_state.fiches_tech["Scène"] == s_s_m)]
                if s_j_m: df_exp = df_exp[df_exp["Jour"] == s_j_m] [cite: 57-69]
                for cat in df_exp["Catégorie"].unique():
                    dico_besoins[f"CATEGORIE : {cat}"] = df_exp[df_exp["Catégorie"] == cat][["Marque", "Modèle", "Quantité", "Groupe"]] [cite: 64-65]
...
with main_tabs[1]:
    ...
    with sub_tabs_tech[0]: [cite: 73]
        if not st.session_state.planning.empty:
            f1, f2, f3 = st.columns(3)
            with f1: sel_j = st.selectbox("📅 Jour", sorted(st.session_state.planning["Jour"].unique()))
            with f2: sel_s = st.selectbox("🏗️ Scène", st.session_state.planning[st.session_state.planning["Jour"] == sel_j]["Scène"].unique()) [cite: 74]
            ...
            if sel_a:
                # --- PDF VIEW ---
                if sel_a in st.session_state.riders_stockage: [cite: 75]
                    riders_groupe = list(st.session_state.riders_stockage[sel_a].keys())
                    if riders_groupe:
                        sel_file = st.selectbox("📂 Voir Rider(s)", ["-- Choisir --"] + riders_groupe, key=f"v_{sel_a}") [cite: 76]
                        if sel_file != "-- Choisir --":
                            pdf_data = st.session_state.riders_stockage[sel_a][sel_file]
                            b64 = base64.b64encode(pdf_data).decode('utf-8')
                            pdf_link = f'<a href="data:application/pdf;base64,{b64}" download="{sel_file}" target="_blank" style="text-decoration:none; color:white; background-color:#FF4B4B; padding:6px 12px; border-radius:5px;">👁️ Ouvrir {sel_file}</a>' [cite: 77-78]
                            st.markdown(pdf_link, unsafe_allow_html=True)

                st.subheader(f"⚙️ Circuits : {sel_a}") [cite: 79]
                ...
                c_c1, c_c2, c_c3, c_c4 = st.columns(4) [cite: 80]
                ...
                st.session_state.artist_circuits[sel_a]["mon_stereo"] = c_c3.number_input("MON Stereo", min_value=0, value=int(st.session_state.artist_circuits[sel_a]["mon_stereo"]), key=f"ms_{sel_a}") [cite: 81]
                ...
                st.subheader(f"📥 Saisie Matériel : {sel_a}") [cite: 82]
                with st.container(border=True):
                    CAT = st.session_state.custom_catalog
                    c_cat, c_mar, c_mod, c_qte, c_app = st.columns([2, 2, 2, 1, 1])
                    v_cat = c_cat.selectbox("Catégorie", list(CAT.keys()) if CAT else ["MICROS", "BACKLINE"]) [cite: 83]
                    v_mar = c_mar.selectbox("Marque", list(CAT[v_cat].keys()) if CAT and v_cat in CAT else ["SHURE"]) [cite: 84]
                    v_mod = c_mod.selectbox("Modèle", CAT[v_cat][v_mar]) if CAT and v_cat in CAT and v_mar in CAT[v_cat] else c_mod.text_input("Modèle") [cite: 85-86]
                    v_qte = c_qte.number_input("Qté", 1, 100, 1) [cite: 87]
                    v_app = c_app.checkbox("Apporte")
                    if st.button("Ajouter"): [cite: 88]
                        new_item = pd.DataFrame([{"Scène": sel_s, "Jour": sel_j, "Groupe": sel_a, "Catégorie": v_cat, "Marque": v_mar, "Modèle": v_mod, "Quantité": v_qte, "Artiste_Apporte": v_app}]) [cite: 90]
                        st.session_state.fiches_tech = pd.concat([st.session_state.fiches_tech, new_item], ignore_index=True)
                        st.rerun()

                df_patch_art = st.session_state.fiches_tech[st.session_state.fiches_tech["Groupe"] == sel_a] [cite: 94]
                st.data_editor(df_patch_art, use_container_width=True, num_rows="dynamic", key=f"ed_{sel_a}", hide_index=True) # FIX : hide_index=True [cite: 94]

    with sub_tabs_tech[1]:
        st.subheader("📋 Patch IN / OUT") [cite: 100]
        if not st.session_state.planning.empty:
            f1p, f2p, f3p = st.columns(3)
            sel_j_p = f1p.selectbox("Jour ", sorted(st.session_state.planning["Jour"].unique()), key="j_p") [cite: 100]
            sel_s_p = f2p.selectbox("Scène ", st.session_state.planning[st.session_state.planning["Jour"] == sel_j_p]["Scène"].unique(), key="s_p") [cite: 101]
            sel_a_p = f3p.selectbox("Groupe ", st.session_state.planning[(st.session_state.planning["Jour"] == sel_j_p) & (st.session_state.planning["Scène"] == sel_s_p)]["Artiste"].unique(), key="a_p") [cite: 102]

            if sel_a_p:
                # --- CALCULS CIRCUITS ---
                def get_c(a, k): return int(st.session_state.artist_circuits.get(a, {}).get(k, 0)) [cite: 103]
                ...
                max_in = 0
                if len(l_art) > 1:
                    for i in range(len(l_art)-1): max_in = max(max_in, get_c(l_art[i], "inputs") + get_c(l_art[i+1], "inputs")) [cite: 107-108]
                else: max_in = get_c(sel_a_p, "inputs") [cite: 105]

                st.metric("Patch Max Scène (Entrées)", max_in) [cite: 109]
                
                # --- TABLEAUX DYNAMIQUES DE PATCH ---
                num_in = get_c(sel_a_p, "inputs")
                if num_in > 0:
                    divisor = 12 # Mode 12N par défaut
                    ...
                    for t_idx in range(nb_tabs):
                        ...
                        ed_df = st.data_editor(
                            ...
                            hide_index=True # FIX : Cache la colonne qui affiche "None"
                        )
